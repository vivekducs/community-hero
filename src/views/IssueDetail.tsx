  ...
  if (!issue) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center max-w-md mx-auto p-6" id="detail-missing">
        ...
        <Link to="/issues" className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg transition-colors">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a 
                        ...
                        className="inline-flex items-center gap-1 text-xs font-bold text-indigo-600 hover:text-indigo-700 mt-2 group"
                      >
                        Open in Google Maps
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6">
            ...
            <div className="relative pl-6 border-l-2 border-slate-100 space-y-8">
              ...
              <div className="relative">
                <div className="absolute -left-[31px] top-1 w-4 h-4 rounded-full bg-indigo-600 ring-4 ring-indigo-50 flex items-center justify-center">
              ...
              <div className="relative">
                <div className={`absolute -left-[31px] top-1 w-4 h-4 rounded-full flex items-center justify-center ${
                  issue.assigned_to ? 'bg-indigo-600 ring-4 ring-indigo-50' : 'bg-slate-200 ring-4 ring-slate-50'
                }`}>
              ...
              <div className="relative">
                <div className={`absolute -left-[31px] top-1 w-4 h-4 rounded-full flex items-center justify-center ${
                  ['resolving', 'in progress', 'resolved'].includes(issue.status.toLowerCase()) ? 'bg-indigo-600 ring-4 ring-indigo-50' : 'bg-slate-200 ring-4 ring-slate-50'
                }`}>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8 shadow-sm space-y-6" id="comments-pane">
            ...
            {user ? (
              <form onSubmit={handleCommentSubmit} className="space-y-3">
                <textarea
                  ...
                  className="w-full min-h-[100px] p-4 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-600/20 focus:border-indigo-600 focus:outline-none transition-all duration-150 resize-none"
                  maxLength={500}
                />
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg shadow-md hover:shadow-indigo-600/20 transition-all duration-150 cursor-pointer disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed"
                  >
                    Post Comment
                  </button>
                </div>
              ...
              <div className="p-4 bg-slate-50 border border-slate-150 rounded-xl text-center">
                <p className="text-xs text-slate-500 font-medium">Please <Link to="/login" className="text-indigo-600 font-bold hover:underline">sign in</Link> to contribute to the discussion.</p>
              </div>
            )}
                  ...
                  <div key={comment.comment_id} className="pt-4 first:pt-0 space-y-2">
                    ...
                    <div className="pl-8 flex items-center gap-3">
                      <button 
                        ...
                        className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-400 hover:text-indigo-600 transition-colors"
                      >
            ...
            {comments.length > commentsLimit && (
              <button
                ...
                className="w-full py-2.5 text-xs font-bold text-indigo-600 hover:text-indigo-700 bg-indigo-50/50 hover:bg-indigo-50 border border-indigo-100 rounded-lg transition-colors flex items-center justify-center gap-1"
              >
                Load More Comments
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6 sticky top-24">
            ...
            <div className="bg-slate-50 rounded-xl p-5 border border-slate-100 space-y-4">
              <div className="flex justify-between items-center text-xs text-slate-500 font-semibold">
                ...
                <span className="text-indigo-600">{issue.verification_percentage}% Verified</span>
              </div>
              <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                <div 
                  className={`h-full transition-all duration-500 rounded-full ${
                    issue.verification_percentage >= 70 ? 'bg-emerald-500' : issue.verification_percentage >= 40 ? 'bg-amber-500' : 'bg-rose-500'
                  }`}
          ...
          <div className="bg-slate-900 text-slate-100 rounded-2xl border border-slate-800 p-6 shadow-md space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="p-3.5 bg-slate-800/60 rounded-xl border border-slate-800 space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-indigo-400 bg-indigo-950/40 px-2 py-0.5 rounded-md font-semibold border border-indigo-900/40">Idempotent</span>
                </div>
                <div className="text-slate-400 space-y-1">
            ...
            {['reported', 'verifying', 'verified', 'investigating', 'resolving'].includes(issue.status) && (
              <button
                ...
                className="w-full py-2.5 text-xs font-bold text-indigo-300 hover:text-white bg-indigo-950/40 hover:bg-indigo-900/40 border border-indigo-800/40 rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer mt-2"
              >
